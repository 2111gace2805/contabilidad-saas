...
export function AccountsReceivable() {
  ...
  const openItems = useMemo(
    () => items.filter((invoice) => Number(invoice.balance ?? 0) > 0 && invoice.status !== 'void'),
    [items]
  );
  ...
  return (
    <div className="p-8 space-y-6">
      <div>
        <h2 className="text-3xl font-bold text-slate-800 mb-2">Cuentas por Cobrar</h2>
        <p className="text-slate-600">Facturas pendientes de cobro</p>
      </div>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-slate-600">No hay cuentas por cobrar pendientes</td>
              </tr>
            ) : (
              openItems.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 text-sm font-medium text-slate-800">{item.invoice_number}</td>
                  <td className="px-4 py-3 text-sm text-slate-700">{item.customer?.name || 'Sin cliente'}</td>
                  <td className="px-4 py-3 text-sm text-slate-700">{new Date(item.invoice_date).toLocaleDateString('es-MX')}</td>
